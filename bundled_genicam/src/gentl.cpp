// gentl.cpp -- nanobind bindings that dynamically load a GenTL Producer
// (".cti") library and expose its C interface (GenTL 1.6) as a fully typed
// Python module.
//
...
std::string error_name(GC_ERROR err) {
    switch (err) {
        ...
        case GC_ERR_AMBIGUOUS: return "GC_ERR_AMBIGUOUS";
        default:
...
NB_MODULE(gentl, m) {
    m.doc() = "Typed Python bindings for the GenICam GenTL 1.6 Producer C "
               "interface. A GenTL Producer (.cti) is loaded dynamically at "
               "runtime via gentl.Producer(path).";
    ...
    nb::enum_<GC_ERROR_LIST>(m, "GC_ERROR", nb::is_arithmetic())
        ...
        .value("GC_ERR_AMBIGUOUS", GC_ERR_AMBIGUOUS)
    ...
    nb::enum_<BUFFER_INFO_CMD_LIST>(m, "BUFFER_INFO_CMD", nb::is_arithmetic())
        ...
        .value("BUFFER_INFO_IS_COMPOSITE", BUFFER_INFO_IS_COMPOSITE)
        .export_values();
    ...
    nb::enum_<PAYLOADTYPE_INFO_IDS>(m, "PAYLOADTYPE_INFO_ID",
        ...
        .value("GENDC", PAYLOAD_TYPE_GENDC)
        .export_values();
    ...
    nb::class_<PyProducer>(
        ...
        "Example\n"
        "-------\n"
        ">>> with gentl.Producer('/path/to/producer.cti') as producer:\n"
