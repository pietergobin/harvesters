"""Full acquisition test for the GenTL 1.5 binding, Producer-controlled
buffer design.
...
def acquiring_stream(stream, remote_registers, payload_size):
    ...
    stream.start_acquisition(gentl15.ACQ_START_FLAGS.DEFAULT, gentl15.GENTL_INFINITE)
...
def test_revoke_while_queued_raises_busy(stream, payload_size):
    ...
    try:
        with pytest.raises(gentl15.GenTLException) as exc_info:
            buf.revoke()
        assert exc_info.value.code == gentl15.GC_ERROR.GC_ERR_BUSY
    finally:
        stream.flush_queue(gentl15.ACQ_QUEUE_TYPE.ALL_DISCARD)
        buf.revoke()
...
def test_get_new_buffer_data_times_out_without_acquisition(stream, payload_size):
    ...
    try:
        ...
        assert exc_info.value.code == gentl15.GC_ERROR.GC_ERR_TIMEOUT
    finally:
        stream.flush_queue(gentl15.ACQ_QUEUE_TYPE.ALL_DISCARD)
        buf.revoke()