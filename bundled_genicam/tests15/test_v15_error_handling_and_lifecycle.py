...
def test_get_info_before_init_lib_raises_not_initialized(cti_path):
    ...
    with pytest.raises(gentl15.GenTLException) as exc_info:
        p.get_info(gentl15.TL_INFO_CMD.TL_INFO_VENDOR)
    assert exc_info.value.code == gentl15.GC_ERROR.GC_ERR_NOT_INITIALIZED


def test_producer_reports_vendor_and_model(producer):
    vendor = producer.get_info(gentl15.TL_INFO_CMD.TL_INFO_VENDOR)
    model = producer.get_info(gentl15.TL_INFO_CMD.TL_INFO_MODEL)
...
def test_open_system_after_init_lib_works(producer):
    ...
    try:
        assert isinstance(
            system.get_info(gentl15.TL_INFO_CMD.TL_INFO_ID), str
        )
    finally:
        system.close()
...
def test_system_enumerates_at_least_one_interface(system):
    system.update_interface_list(gentl15.GENTL_INFINITE)
...
def test_open_interface_with_unknown_id_raises_invalid_id(system):
    ...
    assert exc_info.value.code == gentl15.GC_ERROR.GC_ERR_INVALID_ID
...
def test_get_device_info_reports_vendor_and_model(interface, device_id):
    model = interface.get_device_info(
        device_id, gentl15.DEVICE_INFO_CMD.DEVICE_INFO_MODEL
    )
    vendor = interface.get_device_info(
        device_id, gentl15.DEVICE_INFO_CMD.DEVICE_INFO_VENDOR
    )
    assert model == "Viky"
    assert vendor
...
def test_opening_same_device_twice_raises(interface, device_id):
    first = interface.open_device(
        device_id, gentl15.DEVICE_ACCESS_FLAGS.EXCLUSIVE
    )
    try:
        with pytest.raises(gentl15.GenTLException) as exc_info:
        ...
        assert exc_info.value.code == gentl15.GC_ERROR.GC_ERR_RESOURCE_IN_USE
    finally:
        first.close()
...
def test_data_stream_reports_stream_info(stream):
    assert (
        stream.get_info(gentl15.STREAM_INFO_CMD.STREAM_INFO_IS_GRABBING)
        is False
    )
    assert isinstance(
        stream.get_info(gentl15.STREAM_INFO_CMD.STREAM_INFO_ID), str
    )
...
def test_data_stream_does_not_define_payload_size(stream):
    ...
    assert (
        stream.get_info(gentl15.STREAM_INFO_CMD.STREAM_INFO_DEFINES_PAYLOADSIZE)
        is False
    )
    with pytest.raises(gentl15.GenTLException) as exc_info:
        stream.get_info(gentl15.STREAM_INFO_CMD.STREAM_INFO_PAYLOAD_SIZE)
    assert exc_info.value.code == gentl15.GC_ERROR.GC_ERR_NOT_AVAILABLE